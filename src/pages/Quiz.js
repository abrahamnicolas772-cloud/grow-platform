...
import { CheckCircle, XCircle, Award, ArrowRight, Brain, Target, Code2 } from 'lucide-react';
...
function Quiz() {
  ...
  const loadQuiz = async () => {
    try {
      ...
      if (response.data.type === 'code') {
        setUserCode(response.data.starterCode || '// Write your solution here\n');
      }
    } catch (err) {
  ...
  const evaluateCode = (code, expectedTest) => {
    // Pour la démo, on exécute le code avec une fonction test simple.
    // ⚠️ À ne pas utiliser en production sans sandbox.
    try {
      // On suppose que le code définit une fonction 'add' ou 'multiply'
      const wrapped = `(function() { ${code}; return (${expectedTest}); })()`;
  ...
  const handleSubmit = async () => {
    if (quiz.type === 'qcm') {
      ...
      const isCorrect = evaluateCode(userCode, quiz.expectedTest);
      const score = isCorrect ? 1 : 0;
      setResult({ correct: isCorrect, score });
      setSubmitted(true);
      try {
        await api.submitCodeQuiz(user.id, lessonId, userCode, isCorrect);
      } catch (err) {}
    }
  };
  ...
  // Styles identiques à ceux que vous aviez déjà
  const pageStyle = {
  ...
  if (loading) return <div style={pageStyle}><div style={darkOverlay} /><div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100vh', position: 'relative', zIndex: 1, color: 'white' }}>Loading quiz...</div></div>;
  if (error) return <div style={pageStyle}><div style={darkOverlay} /><div style={{ ...containerStyle, textAlign: 'center', color: '#ef4444' }}>{error}</div></div>;
  if (!quiz) return <div style={pageStyle}><div style={darkOverlay} /><div style={{ ...containerStyle, textAlign: 'center', color: '#94a3b8' }}>No quiz available</div></div>;
      ...
      <div style={containerStyle}>
        <div style={cardStyle}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '1rem' }}>
            {quiz.type === 'code' ? <Code2 size={22} color="#60a5fa" /> : <Brain size={22} color="#60a5fa" />}
            ...
            <>
              ...
              {quiz.type === 'qcm' ? (
                ...
                <div style={{ marginBottom: '1rem' }}>
                  <p style={{ color: '#94a3b8', marginBottom: '0.5rem', fontSize: '0.85rem' }}>Write your code below:</p>
                  <CodeEditor code={userCode} setCode={setUserCode} language="javascript" />
                </div>
              )}
              <button style={{ ...buttonStyle, opacity: (quiz.type === 'qcm' && !selectedAnswer) ? 0.5 : 1 }} onClick={handleSubmit} disabled={quiz.type === 'qcm' && !selectedAnswer}>
