...
function Home() {
  ...
  // --- État pour les vrais cours ---
  const [courses, setCourses] = useState([]);
  const [loadingCourses, setLoadingCourses] = useState(true);

  // FAQ statique (inchangée)
  const faq = [
  ...
  useEffect(() => {
    const loadCourses = async () => {
      try {
        ...
        // Enrichir avec des données fictives pour affichage (rating, étudiants, durée)
  ...
  // Filtrer les cours selon le terme de recherche (si nécessaire)
  const filteredCourses = courses.filter(course =>
    course.title.toLowerCase().includes(searchTerm.toLowerCase()) ||
    (course.description && course.description.toLowerCase().includes(searchTerm.toLowerCase()))
  );

  // Les cours à afficher dans "Popular Courses" : les 3 premiers (ou tous si moins)
  ...
  // Handlers (chat, newsletter, etc.) inchangés
  ...
  if (loadingCourses) {
    return (
      <div style={{ background: '#0a0a0f', minHeight: '100vh', display: 'flex', justifyContent: 'center', alignItems: 'center', color: 'white' }}>
  ...
  return (
    <div style={{ background: '#0a0a0f', color: 'white', fontFamily: "'Inter', sans-serif", overflowX: 'hidden' }}>
      <style>
        {`
          ...
          @media (max-width: 768px) {
            ...
            .hero-image { max-height: 300px !important; }
      ...
      <section style={{ padding: '80px 20px', background: 'linear-gradient(135deg, #0a0a0f 0%, #0d1b3e 100%)' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '4rem', alignItems: 'center' }}>
          ...
          <div style={{ background: '#0f172a', borderRadius: '1.5rem', overflow: 'hidden', boxShadow: '0 20px 30px -12px rgba(0,0,0,0.3)' }}>
            <img src="https://i.postimg.cc/tTskRqTc/photo-2026-05-26-10-31-47.jpg" alt="About GROW Platform" style={{ width: '100%', height: 'auto', objectFit: 'cover', display: 'block' }} />
          </div>
        </div>
      </section>
          ...
          {popularCourses.map((course) => (
            ...
            >
              ...
              <div style={{ position: 'relative', overflow: 'hidden' }}>
                <img
                  src={course.image_url || `https://picsum.photos/id/${(course.id * 10) % 100}/400/250`}
                  alt={course.title}
                  style={{
                    width: '100%',
                    height: '220px',
              ...
              <div style={{ padding: '1.5rem' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.75rem' }}>
                  ...
                  <div style={{
                    ...
                    <span style={{ color: '#e2e8f0', fontSize: '0.8rem', fontWeight: '600' }}>{course.rating?.toFixed(1)}</span>
                  </div>
                </div>
                <p style={{ color: '#94a3b8', fontSize: '0.85rem', marginBottom: '1rem', lineHeight: '1.5' }}>{course.description?.substring(0, 100)}...</p>
                ...
                <div style={{ marginTop: '0.5rem', marginBottom: '0.5rem' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.6rem', color: '#64748b', marginBottom: '0.25rem' }}>
                    <span>Completion rate</span>
                    <span>87%</span>
                  </div>
                  <div style={{ width: '100%', height: '3px', background: 'rgba(255,255,255,0.1)', borderRadius: '2px', overflow: 'hidden' }}>
                    <div style={{ width: '87%', height: '100%', background: 'linear-gradient(90deg, #60a5fa, #06b6d4)', borderRadius: '2px' }} />
                  </div>
                </div>
                {/* Prix et bouton */}
                <div style={{
                  ...
                  <div>
                    ...
                    <div>
                      <span style={{ fontSize: '1.6rem', fontWeight: 'bold', color: '#60a5fa' }}>$29</span>
                  ...
                  <button
                    ...
                    onMouseEnter={e => { e.target.style.transform = 'translateY(-2px)'; e.target.style.boxShadow = '0 6px 15px rgba(59,130,246,0.4)'; }}
      ...
      <section style={{ padding: '80px 20px', background: 'linear-gradient(135deg, #0a0a0f 0%, #0d1b3e 100%)', position: 'relative', overflow: 'hidden' }}>
